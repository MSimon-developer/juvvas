...
import { Home, Heart, Calendar, MessageSquare, Plus, Eye } from "lucide-react";
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      try {
        ...
        // 🔹 Listings
        const resListings = await fetch("http://127.0.0.1:8000/my-properties/", { headers });
        const listingsData = resListings.ok ? await resListings.json() : [];

        // 🔹 Bookings
        const resBookings = await fetch("http://127.0.0.1:8000/bookiings/", { headers });
        const bookingsData = resBookings.ok ? await resBookings.json() : [];

        // 🔹 Favorites
        const resFav = await fetch("http://127.0.0.1:8000/favorites/", { headers });
